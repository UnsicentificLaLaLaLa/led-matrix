...
void SPI_SetBaudrate(SPI_Type * SPIx, uint32_t src_clk, uint32_t baudrate)
{
    ...
    if (div <= 4)
    {
        ...
        SPIx->CCTL |= (SPI_CCTL_TXEDGE_MASK | SPI_CCTL_RXEDGE_MASK);
    }
    else
    {
        SPIx->CCTL &= ~(SPI_CCTL_TXEDGE_MASK | SPI_CCTL_RXEDGE_MASK);
...
{
    uint32_t gctl = SPIx->GCTL &~ ( SPI_GCTL_MODE_MASK
                                  | SPI_GCTL_RXEN_MASK
                                  | SPI_GCTL_TXEN_MASK
                                  | SPI_GCTL_NSS_MASK
                                  | SPI_GCTL_INTEN_MASK
    ...
    uint32_t cctl = SPIx->CCTL &~ ( SPI_CCTL_CPHA_MASK
                                  | SPI_CCTL_CPOL_MASK
                                  | SPI_CCTL_LSBFE_MASK
                                  | SPI_CCTL_SPILEN_MASK
                                  );

    /* Master. */
    gctl |= SPI_GCTL_MODE_MASK; /* master mode, disable spi, and reset the other bits in CCTL. */

    /* XferMode. */
    switch (init->XferMode)
    {
        case SPI_XferMode_RxOnly:
            gctl |= SPI_GCTL_RXEN_MASK;
            break;

        case SPI_XferMode_TxRx:
            gctl |= (SPI_GCTL_RXEN_MASK | SPI_GCTL_TXEN_MASK);
            break;
    ...
    if (init->AutoCS)
    {
        gctl |= SPI_GCTL_NSS_MASK;
    }

    /* Interrupts. Always enable the global interrupt. The specific events are controlled by each bits in INTEN register. */
    gctl |= SPI_GCTL_INTEN_MASK;

    /* DataWidth. */
    if (init->DataWidth == SPI_DataWidth_7b)
    {
        extctl = SPI_EXTCTL_EXTLEN(init->DataWidth);
    }
    else if (init->DataWidth == SPI_DataWidth_8b)
    {
        cctl |= SPI_CCTL_SPILEN_MASK;
        extctl = SPI_EXTCTL_EXTLEN(init->DataWidth);
    }
    else
    {
        gctl |= SPI_GCTL_DW832_MASK;
        extctl = SPI_EXTCTL_EXTLEN(init->DataWidth);
    }

    /* CPOL & CPHA. */
    cctl |= ( (SPI_CCTL_CPHA_MASK | SPI_CCTL_CPOL_MASK)
            & ((init->PolPha) << SPI_CCTL_CPHA_SHIFT)
            );

    /* MSB. */
    if (init->LSB)
    {
        cctl |= SPI_CCTL_LSBFE_MASK;
    }
